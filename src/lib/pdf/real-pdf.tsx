...
export const InvoiceWeb = ({ invoiceId }: InvoiceWebProps) => {
  ...
  if (invoiceQuery.isLoading || companyQuery.isLoading) {
    return (
      <div className="text-center py-10 text-gray-600">Loading invoice...</div>
    );
  }

  // Error
  if (invoiceQuery.isError || companyQuery.isError) {
    return (
  ...
  if (!invoiceData || !companyData) {
    return (
      <div className="text-center py-10 text-gray-600">No data available.</div>
    );
  }
  ...
  return (
    <>
      <div className=" w-full h-full flex flex-col gap-y-5">
        ...
        <div
          id="page1"
          className="mx-auto bg-gray-50 flex divide-x
             w-[794px] h-[1123px] min-h-[1120px] max-h-[1120px]
"
        >
          <GarmentCard
        ...
        <div
          id="page2"
          className="mx-auto bg-gray-50 flex divide-x items-stretch
             w-[794px] h-[1123px] min-h-[1120px] max-h-[1120px]
"
        >
          {/* Garments */}
            ...
            <div className=" flex flex-col py-11 w-full px-4 gap-y-10 ">
              <div className=" border-border/30 border-b rounded-lg flex flex-col gap-y-6 justify-between pb-9 ">
                <div className=" flex flex-col gap-y-2">
                  ...
                  <p className="text-sm text-accent">{address}</p>
                  <p className="text-sm text-accent">Tel: {phoneNumber}</p>
                  {email && (
                    <p className="text-sm text-accent">Email: {email}</p>
                  )}
                  {websiteUrl && (
                    <p className="text-sm text-accent">Website: {websiteUrl}</p>
                  )}
                  {whatsappNumber && (
                ...
                <div className=" flex flex-col gap-y-2">
                  ...
                  <p className="text-sm text-muted">
                    Issue Date: {formatDate(invoiceData.date)}
                  </p>
                </div>
              </div>
              <div className="mt-6 pt-6 w-full flex flex-col justify-between items-stretch px-8">
                ...
                {saleMen && saleMen.length > 0 && (
                  <div className="flex justify-between  mb-2">
                    ...
                    <span className="font-semibold text-accent">
                      <p className=" text-sm text-end text-balance">
